...
import { fetchRules, KeywordRuleParams, Rule, RulesStore, saveRules } from "@/lib/api";
import RuleEditor from "@/components/RuleEditor";

function newKeywordRule(): Rule<KeywordRuleParams> {
  return {
    id: `kw-${Date.now()}`,
    name: "새 키워드 규칙",
    enabled: true,
...
const EXTRA = [
  { key: "minImpressions", label: "최소 노출", suffix: "회" },
  { key: "lowImpBoost", label: "저노출 증액", suffix: "%" },
  { key: "lowRankThreshold", label: "저순위 기준", suffix: "위" },
  { key: "lowRankCtrThreshold", label: "저순위 CTR 기준", suffix: "%" },
  { key: "lowRankBoost", label: "저순위 증액", suffix: "%" },
];
...
export default function KeywordRulesPage() {
  ...
  function update(idx: number, r: Rule<KeywordRuleParams>) {
    setStore((s) => s && { ...s, keyword: s.keyword.map((x, i) => i === idx ? r : x) });
  }
  function remove(idx: number) {
    if (!confirm("이 규칙을 삭제할까요?")) return;
    setStore((s) => s && { ...s, keyword: s.keyword.filter((_, i) => i !== idx) });
  }
  function add() { setStore((s) => s && { ...s, keyword: [...s.keyword, newKeywordRule()] }); }
    ...
    <div className="p-8">
      <div className="max-w-[980px] mx-auto">
        <header className="mb-6">
          ...
          <h1 className="text-display mt-1">키워드 규칙</h1>
          <p className="text-caption mt-2" style={{ color: "var(--color-text-muted)" }}>
            키워드 입찰가 자동 추천의 기준 · 30일 데이터 기반
          </p>
        </header>

        {store.keyword.map((r, i) => (
          <RuleEditor
            key={r.id}
            rule={r}
